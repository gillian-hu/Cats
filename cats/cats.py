...
def about(subject):
    ...
    def paragraph_checker(paragraph):
        x = remove_punctuation(paragraph)
        x = lower(x)
        x = split(x)
        for word in subject:
            if word in x:
...
def wpm(typed, elapsed):
    ...
    return round(60/elapsed * num_words, 2)
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    small_diff = diff_function(typed_word, word_list[0],limit)
    word_stored = word_list[0]
...
def report_progress(typed, source, user_id, upload):
    ...
    while i < len(typed):
        if typed[i] == source[i]:
            ...
            break
        i += 1
    d = {'id': user_id, 'progress': number_correct/len(source)}
    upload(d)
    return number_correct/len(source)
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[] for p in range(len(timestamps_per_player))]  # You may remove this line
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    tracker = [[] for p in range(len(times))]
    for i in word_indices:
        ...
        for j in range(len(times)):
            if times[j][i] < lowest:
                lowest = times[j][i]
                lowest_player = j
        tracker[lowest_player].append(words[i])
    return tracker
    # END PROBLEM 10
